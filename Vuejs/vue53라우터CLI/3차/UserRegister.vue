...
export default {
    ...
    data() {
        return {
            ...
            errId: false /*아이디 유효성 체크. 4자이상 6자 이내 */,
            errPwd: false /**비번 유효성 체크. 4자이상 8자 이내 */,
            errPwd2: false /**비번 재확인 */,
            errEmail: false,
    ...
    methods: {
        ...
        checkPwd: function () {
            ...
            this.errPwd = len < 4 || len > 8 ? true : false;
        },
        checkRePwd: function (e) {
            let pwd2 = e.target.value; //재확인 비번값
            this.errPwd2 = this.user.pwd != pwd2 ? true : false;
        },
        checkEmail: function () {
            var pattern = /^[\w-_]+(\.[\w]+)*@([a-zA-Z]+\.)+[a-z]{2,3}$/;
            this.errEmail = pattern.test(this.user.email) ? false : true;
        },
        join: function (e) {
